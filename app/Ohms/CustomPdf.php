...
class CustomPdf
{
     ...
     * @param array  $config
     */
    public static function __prepare(object $cacheFile, array $config)
    {
...
EOD;
        ...
        $url = Utils::protocol().'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?cachefile={$qs['cachefile']}";
...
EOD;
     ...
     * Get Index HTML
     *
     * @param object $indexPoints
     * @return string
     */
    public static function getIndexHtml(object $indexPoints): string
    {
        parse_str($_SERVER['QUERY_STRING'], $qs);
        $indexHTML = "";
        foreach ($indexPoints->point as $point) {
            ...
            $indexHTML .= '<div style="text-align:justify;font-size:10px; line-height: 5px;font-weight:bold;text-align:left;"><a  style="line-height: 15px;font-size:12px;font-weight:bold;" href="'.Utils::protocol().'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?cachefile={$qs['cachefile']}".'#segment'.$point->time.'" id="link'.$point->time.'"><b>'.$timePoint.' - '.trim($title, ';')."</b></a></div>";
    ...
    /**
     * Convert seconds to [hours, minutes] associative array.
     *
     * @param int $time
     * @return array
     */
    protected static function convertToHoursMins(int $time): array
    {
        $hours   = "00";
        $minutes = "00";
        if ($time > 0) {
     ...
     * @param string $syncPoints
     * @param string $url
     * @return string
     */
    protected static function getFormattedTranscript(array $sections, string $syncPoints, string $url): string
    {
        $intervalIncrement = 1;
        if (!empty($syncPoints)) {
        ...
        foreach ($syncArray as $points) {
            ...
            if ($intervalIncrement == 0.50) {
                ...
                [$hours, $mins] = self::convertToHoursMins($timestampCounter);
