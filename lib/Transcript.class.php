...
class Transcript {
    ...
    private function formatIndex()
    {
        if (!empty($this->index)) {
            ...
            foreach ($this->index->point as $point) {
                ...
                $partialTranscript = $point->partialTranscript;
                $keywords = $point->keywords;
                $subjects = $point->subjects;
                $gps = $point->gps;
                $gpsText = $point->gpsText;
                $hyperlink = $point->hyperlink;
                $hyperlinkText = $point->hyperlinkText;
                ...
                $indexHTML .= '<p><strong>Partial Transcript:</strong> <span>' . $partialTranscript . '</span></p><p><strong>Segment Synopsis:</strong><span> ' . $synopsis . '</span></p><p><strong>Keywords:</strong><span> ' . str_replace(';', '; ', $keywords) . '</span></p><p><strong>Subjects:</strong><span> ' . str_replace(';', ' ', $subjects) . '</span></p>';
                if ($gps <> '') {
                ...
                if ($hyperlink <> '') {
                    $indexHTML .= '<br/><strong>Hyperlink:</strong> <a class="fancybox" rel="group" target="_new" href="' . $hyperlink . '">' . $hyperlinkText . '</a><br/>';
                }
                $indexHTML .= '</div>';
                $indexHTML .= "\n</div>\n";
            }
    ...
    {
        # quote kw for later
        $quotedWords = $this->quoteWords($keyword);
        $json = "{ \"keyword\":\"$quotedWords\", \"matches\":[";
